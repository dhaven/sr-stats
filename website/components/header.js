...
import { ChevronDownIcon, WrenchScrewdriverIcon, TrophyIcon, ArrowRightOnRectangleIcon, UserCircleIcon } from '@heroicons/react/24/outline'
    ...
    return (
        <>
            <header className="z-10 bg-white flex flex-row drop-shadow-md lg:drop-shadow-scifi5 justify-center p-2 md:p-4 gap-1">
                ...
                {
                    pathname == "/game/[id]" &&
                    <button onClick={(e) => { setAddGameIsOpen(true) }} className="hidden md:block absolute top-0 right-0 m-3 text-scifi5 font-medium bg-white text-sm px-2 sm:px-4 py-2 border border-scifi4 ring-scifi2 drop-shadow-md hover:ring rounded-lg" type="button">
                        New Game
                    </button>
                }
                {
                    pathname == "/" && !session &&
                    <button onClick={(e) => { setIsLoginOpen(true) }} className="hidden md:block absolute top-0 right-0 m-3 text-scifi5 font-medium bg-white text-sm px-2 sm:px-4 py-2 border border-scifi4 ring-scifi2 drop-shadow-md hover:ring rounded-lg" type="button">
                        Sign in
                    </button>
                }
                {session?.user && (
                    <div className="absolute top-0 right-0">
                        <Menu as="div" className="">
                            <div>
                                <Menu.Button className="inline-flex items-center mt-3 mr-3 text-scifi5 font-medium bg-white text-sm px-2 sm:px-4 py-2 border border-scifi4 ring-scifi2 drop-shadow-md hover:ring rounded-2xl">
                                    {session.user.image ? (
                                        <Image
                                            alt="profile_picture"
            ...
            <AddIGN isOpen={isAddIGNOpen} setIsOpen={setIsAddIGNOpen}></AddIGN>
        </>
    )
}
