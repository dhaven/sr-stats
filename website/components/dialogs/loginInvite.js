...
function SRLoginInvite({ isOpen, setIsOpen, gameSummary }) {
    ...
    function emailLogin(email) {
        signIn("email", 
            ...
            .then(res => {
                if(res.ok){ //ask user to check his emails
                    console.log("check emails please")
                }
                console.log(res)
                setEmail("")
            })
                            ...
                            <div className="inline-block justify-center w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                                ...
                                <div className="flex flex-row justify-center">
                                    <div className="w-3/4 flex flex-col items-center gap-2">
                                        ...
                                        <input placeholder="Enter your email" className="w-full p-2 border drop-shadow-md border-scifi4 ring-scifi2 hover:ring text-sm rounded-lg resize-none" type="text" value={email} onChange={handleEmailChange} />
                                        <button
                                            onClick={() => emailLogin(email)}
                                            className="w-full text-scifi5 font-medium bg-white text-sm px-2 sm:px-4 py-2 border border-scifi4 ring-scifi2 drop-shadow-md hover:ring rounded-lg"
                                        >
                                            Sign in with Email
                                        </button>
                                    </div>
